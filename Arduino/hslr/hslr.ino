/* v1.6.2VIIIRS GYBE,LLC HSLR code to run Hamamatsu C12880MA with Feather M0 (+BLE) datalogger
...
#include <Time.h>             // System datetime library
...
uint16_t millis_count;      // Millisecond counter that rollsover every second
uint32_t millis_ref;        // Millisecond reference for computing millis_count
...
uint32_t datetime;            // Hold date/time in epoch format (NofSecs since Jan 01, 1970)
...
void readSpec() {
  ...
  //  millis_count = millis();    // OZ - not needed, done in read clock function
...
//  Read RTC clock, and compute milliseconds since start of current clock.sec
  ...
  datetime = rt_now.unixtime();   // epoch format
...
uint16_t SetMillisRef(uint32_t millisin) {
  ...
  // compute milliseconds since start of current clock.sec using new ref
  uint16_t postmillis = (millisin - NewMillisRef) % 1000;

  // Not Common - Current millis less than (behind) the new ms reference
  if (postmillis < 0) {
    // Gap to reach Max value should be added (total < 1000), i.e. use the previous clock.sec marker
    postmillis = msGap2MaxValue + millisin;
  }
...
/* Initialize RTC modeule
  //-----------------------------*/
void initializeRTC() {
  if (! rtc.begin()) {    //OZ this function always return true after Wire.begin() is excecuted, may not be useful
  ...
  // Initialize system clock to start at 0 ms, in order to get the correct millisecond reference
  DateTime cnow = rtc.now();  // Get datetime from RTC
  setTime(cnow.unixtime());   // Set system clock
  millis_ref = millis();      // Initialize millisecond reference
  //uint32_t millis_start = millis();
  //uint32_t millis_end = millis();
  //millis_ref = millis() - (millis_end - millis_start);
  //millis_ref = millis();
